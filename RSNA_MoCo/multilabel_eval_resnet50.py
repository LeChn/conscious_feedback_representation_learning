...
from models.resnet_random import resnet50
import argparse
...
def main():
    ...
    #shuffle(trainfiles)
    #trainfiles = trainfiles[:int(0.0005 * len(trainfiles))]
    csv_label = "train.csv"
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        time1 = time.time()
        train_auc = train(epoch, train_loader, model, criterion, optimizer, args)
        time2 = time.time()
        print('train epoch {}, total time {:.2f}'.format(epoch, time2 - time1))
        print("==> testing...")
        auc, mean_auc, test_loss = validate_multilabel(val_loader, model, criterion, args)
...
def validate_multilabel(val_loader, model, criterion, opt):
    ...
    with torch.no_grad():
        ...
        for idx, (input, target) in enumerate(val_loader):
            ...
            outPRED = torch.cat((outPRED, output.data), 0)
            loss = criterion(output, target.float())
            #Logloss, mean_ll = compute_Logloss(outGT, outPRED, 6)
            losses.update(loss.item(), input.size(0))
            batch_time.update(time.time() - end)
            end = time.time()
