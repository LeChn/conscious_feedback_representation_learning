...
def max_pool_layer2d(x, kernel_size=(2, 2), strides=(2, 2), padding="SAME"):
    ...
    op = tf.nn.max_pool(x, ksize=kernel_size_aug,
                        strides=strides_aug, padding=padding)
...
def max_pool_layer3d(x, kernel_size=(2, 2, 2), strides=(2, 2, 2), padding="SAME"):
    ...
    op = tf.nn.max_pool3d(x, ksize=kernel_size_aug,
                          strides=strides_aug, padding=padding)
...
def crop_and_concat_layer(inputs, axis=-1):
    ...
    for ii in range(1, len(inputs)):
        ...
        if len(output_size) == 5:  # 3D images
            ...
            raise ValueError(
                'Unexpected number of dimensions on tensor: %d' % len(output_size))
...
def conv2D_layer(bottom,
    ...
    weight_shape = [kernel_size[0], kernel_size[1],
                    bottom_num_filters, num_filters]
    ...
    with tf.variable_scope(name):
        ...
        weights = get_weight_variable(
            weight_shape, name='W', type=weight_init, regularize=True)
        op = tf.nn.conv2d(bottom, filter=weights,
                          strides=strides_augm, padding=padding)
...
def conv3D_layer(bottom,
    ...
    weight_shape = [kernel_size[0], kernel_size[1],
                    kernel_size[2], bottom_num_filters, num_filters]
    bias_shape = [num_filters]
    ...
    with tf.variable_scope(name):
        ...
        weights = get_weight_variable(
            weight_shape, name='W', type=weight_init, regularize=True)
        op = tf.nn.conv3d(bottom, filter=weights,
                          strides=strides_augm, padding=padding)
...
def deconv3D_layer(bottom,
    ...
    if output_shape is None:
        output_shape = tf.stack([bottom_shape[0], bottom_shape[1]*strides[0],
                                 bottom_shape[2]*strides[1], bottom_shape[3]*strides[2], num_filters])

    bottom_num_filters = bottom_shape[4]

    weight_shape = [kernel_size[0], kernel_size[1],
                    kernel_size[2], num_filters, bottom_num_filters]
...
def conv2D_dilated_layer(bottom,
    ...
    with tf.variable_scope(name):
        ...
        op = tf.nn.atrous_conv2d(
            bottom, filters=weights, rate=rate, padding=padding)
...
def dense_layer_bn(bottom,
    ...
    batchnorm = batch_normalisation_layer(
        linact, name + '_bn', training=training)
...
def get_weight_variable(shape, name=None, type='xavier_uniform', regularize=True, **kwargs):
    ...
    if type == 'xavier_uniform':
        ...
        initial = variance_scaling_initializer(
            uniform=False, factor=2.0, mode='FAN_IN', dtype=tf.float32)
    elif type == 'he_uniform':
        initial = variance_scaling_initializer(
            uniform=True, factor=2.0, mode='FAN_IN', dtype=tf.float32)
    elif type == 'caffe_uniform':
        initial = variance_scaling_initializer(
            uniform=True, factor=1.0, mode='FAN_IN', dtype=tf.float32)
    elif type == 'simple':
...
def _bilinear_upsample_weights(shape):
    ...
    if not shape[0] == shape[1]:
        raise ValueError('kernel is not square')
    if not shape[2] == shape[3]:
        raise ValueError(
            'input and output featuremaps must have the same size')
